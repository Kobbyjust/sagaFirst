...
import { deleteFootwears, listFootwears, saveFootwears } from '../../actions/AdminActions/addFootwears';
...
function FootwearsScreen(props) {

    const [products, setProduct] = useState([]);
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/Footwears'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveFootwears(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteFootwears(product.name));
      };
...
export default FootwearsScreen;