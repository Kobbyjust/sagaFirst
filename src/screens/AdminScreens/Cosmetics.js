...
//import { deleteCosmetics, listCosmetics, saveCosmetics } from '../actions/productActions';
import Axios from 'axios';
import { deleteCosmetics, listCosmetics, saveCosmetics } from '../../actions/AdminActions/addCosmetics';

  
   
function CosmeticsScreen(props) {

    const [products, setProduct] = useState([]);
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/Cosmetics'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveCosmetics(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteCosmetics(product.name));
      };
