...
import { deletePhones, listPhones, savePhones } from '../../actions/AdminActions/addPhone';
//import { deletePhones, listPhones, savePhones } from '../actions/productActions';
...
function ProvisionsScreen(props) {
    ...
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/Phones'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(savePhones(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deletePhones(product.name));
      };
