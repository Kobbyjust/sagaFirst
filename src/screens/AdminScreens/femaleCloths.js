...
import { deleteFemaleCloths, listFemaleCloths, saveFemaleCloths } from '../../actions/AdminActions/addFemaleCloth';
...
function ProvisionsScreen(props) {
    ...
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/FemaleCloths'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveFemaleCloths(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteFemaleCloths(product.name));
      };
