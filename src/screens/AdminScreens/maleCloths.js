...
import { deleteMaleCloths, listMaleCloths, saveMaleCloths } from '../../actions/AdminActions/addMaleCloth';
...
function ProvisionsScreen(props) {
    ...
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/MaleCloths'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveMaleCloths(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteMaleCloths(product.name));
      };
