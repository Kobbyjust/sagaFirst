...
//import { deleteBags, listBags, saveBags } from '../actions/productActions';
import Axios from 'axios';
import { deleteBags, listBags, saveBags } from '../../actions/AdminActions/addBag';

  
   
function BagsScreen(props) {

    const [products, setProduct] = useState([]);
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/Bags'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveBags(name, price, type, image, description, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteBags(product.name));
      };
