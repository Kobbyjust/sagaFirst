...
import { deleteProvisions, listProvisions, saveProvisions } from '../../actions/AdminActions/addProvision';
...
function ProvisionsScreen(props) {
    ...
    useEffect (() => {
        const fetchData = async () => {
            const {data} = await  Axios.get('/api/product/Provisions'); 
            setProduct(data); 
        }
        fetchData()
        return() =>{   
    ...
    const dispatch = useDispatch();
       ...
       const submitHandler = (e) =>{
           e.preventDefault();
           dispatch(saveProvisions(name, price, type, image, description,brand, countInStock));

       }
       const deleteHandler = (product) => {
        dispatch(deleteProvisions(product.name));
      };
            ...
            <div class="col-2">
                <div class="form-shs-container">
                    ...
                    <form id="OrderForm" onSubmit={submitHandler}>
                        ...
                        <textarea name="brand"  cols="49" rows="7" class="FormStyle"   placeholder="Product Brand"  required  onChange={(e) => setBrand(e.target.value)}></textarea>
                        <br></br>
