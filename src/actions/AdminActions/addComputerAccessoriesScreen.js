...
export const listComputerAccessories = () => async (dispatch ) => {
    try {
        dispatch(PRODUCT_LIST_REQUEST);
        const  {shop_Books} = await Axios.get('/api/product/ComputerAccessories'); 
...
export const saveComputerAccessories = (name, price, type, image, description, countInStock) => async (dispatch, getState) =>{

    //original
    try {
        dispatch({type: PRODUCT_SAVE_REQUEST, payload:{name, price, type, image, description, countInStock}});
        const{data} = await Axios.post('/api/product/ComputerAccessories', {name, price, type, image, description, countInStock},
        ); 
...
export const deleteComputerAccessories = (productId) => async (dispatch, getState) =>{ 
    try {
