...
export const listBags = () => async (dispatch ) => {
    try {
        dispatch(PRODUCT_LIST_REQUEST);
        const  {shop_Books} = await Axios.get('/api/product/Bags'); 
...
export const saveBags = (name, price, type, image, description, countInStock) => async (dispatch, getState) =>{

    //original
    try {
        dispatch({type: PRODUCT_SAVE_REQUEST, payload:{name, price, type, image, description, countInStock}});
        const{data} = await Axios.post('/api/product/Bags', {name, price, type, image, description, countInStock},
        ); 
...
export const deleteBags = (productID) => async (dispatch, getState) =>{ 
    try {
