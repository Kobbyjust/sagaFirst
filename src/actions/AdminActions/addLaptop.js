...
export const listLaptops = () => async (dispatch ) => {
    try {
        ...
        const  {shop_Books} = await Axios.get('/api/product/Laptops'); 
...
export const saveLaptops = (name, price, type, image, description, countInStock) => async (dispatch, getState) =>{

    //original
    try {
        dispatch({type: PRODUCT_SAVE_REQUEST, payload:{name, price, type, image, description, countInStock}});
        const{data} = await Axios.post('/api/product/Laptops', {name, price, type, image, description, countInStock},
        ); 
...
export const deleteLaptops = (productId) => async (dispatch, getState) =>{ 
    try {
