...
export const listElectronics = () => async (dispatch ) => {
    try {
        ...
        const  {shop_Books} = await Axios.get('/api/product/Electronics'); 
...
export const saveElectronics = (name, price, type, image, description, countInStock) => async (dispatch, getState) =>{

    //original
    try {
        dispatch({type: PRODUCT_SAVE_REQUEST, payload:{name, price, type, image, description, countInStock}});
        const{data} = await Axios.post('/api/product/Electronics', {name, price, type, image, description, countInStock},
        ); 
...
export const deleteElectronics = (productID) => async (dispatch, getState) =>{ 
    try {
