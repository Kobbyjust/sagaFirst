...
export const listCosmetics = () => async (dispatch ) => {
    try {
        dispatch(PRODUCT_LIST_REQUEST);
        const  {shop_Books} = await Axios.get('/api/product/Cosmetics'); 
...
export const saveCosmetics = (name, price, type, image, description, countInStock) => async (dispatch, getState) =>{

    //original
    try {
        dispatch({type: PRODUCT_SAVE_REQUEST, payload:{name, price, type, image, description, countInStock}});
        const{data} = await Axios.post('/api/product/Cosmetics', {name, price, type, image, description, countInStock},
        ); 
...
export const deleteCosmetics = (productID) => async (dispatch, getState) =>{ 
    try {
